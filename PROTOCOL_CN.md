...
- **examples 强制脱敏**：所有进入公开示例的数据必须先脱敏——公司/品牌/供应商名一律泛化，人名替换为角色，金额改为相对描述，内部项目代号与路径不得出现；结构化数值（O/I/C1/C2/状态）与脱敏后的介入示例可保留；
- 发布前须做敏感词扫描（公司名/供应商/人名/金额/代号），任一命中即须先脱敏；完整规则见 [examples/PILOT_CASE_CN.md](examples/PILOT_CASE_CN.md) 末尾；
